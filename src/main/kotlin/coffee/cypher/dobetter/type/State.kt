...
fun interface State<S, A> : Monad<A> {
    fun run(state: S): Pair<S, A>

    class Type<S> : Monad.Type {
...
fun <S, T> state(f: suspend ComputationContext<T>.() -> T): State<S, T> = State.Type<S>().evaluate(f)
